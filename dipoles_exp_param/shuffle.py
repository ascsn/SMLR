...
# -----------------------------
random.seed(SEED) # this is seed for shuffling data
np.random.seed(PARAM_SEED) # this is seed for shuffling params
...
def alphaD_rel_errors(params, eval_points_str, strength_true, alphaD_true, *,
    ...
    """
    # Call the same cost function, but we only care about alphaD_calc here
    total, Ls, Lm1, Lmplus1, _, _, _, alphaD_calc_tf, _, _ = helper.cost_function_batched_mixed(
        params, n,
        eval_points_str,
        strength_true,
        alphaD_true,
...
# -----------------------------
# Helpers to get central fit and init
# -----------------------------
keep = round(retain * n)

# Load central strength and fit (E_hat, B_hat)
...
# Set some convenient seed defaults you had
...
# Retain-aware init (fills center with E_hat,B_hat, zeros v0 elsewhere, stepped D ends)
initial_guess = helper.encode_initial_guess(base_random, E_hat, B_hat, n, retain)
...
@tf.function
def train_step(params, train_set_pts, strength_true, alphaD_true):
    # snapshot params BEFORE the update to measure how much they move
    old_params = tf.identity(params)

    with tf.GradientTape() as tape:
        total, Ls, Lm1, Lmplus1, Lor, Lor_true, xgrid, alphaD_calc, B_last, E_last = \
            helper.cost_function_batched_mixed(
                params, n,
                train_set_pts,                # fmt_data (list of (alpha,beta))
                strength_true,                # list of (G,2)
                alphaD_true,                  # list/array (B,)
                central_point,                # (alpha0, beta0)
                retain,                       # retain fraction
                w_strength=W_STRENGTH,
                w_mminus1=W_MMINUS1,
                w_mplus1=W_MPLUS1,
                m1_target=M1_TARGET,
    ...
    return total, Ls, Lm1, Lmplus1, grad_norm, param_delta, Lor, Lor_true, xgrid, alphaD_calc, B_last, E_last


def evaluate_on(params, eval_points_str, strength_true, alphaD_true):
    ...
    return float(total.numpy()), float(Ls.numpy()), float(Lm1.numpy()), float(Lmplus1.numpy())
...
for tier_idx, tier_points_str in enumerate(train_tiers_str, start=1):
    ...
    if not WARM_START:
        params.assign(initial_guess)

    # Train on this tier
    costs_hist = []
    for it in range(1, NUM_ITERS_TIER + 1):
        total, Ls, Lm1, Lmplus1, gnorm, pdelta, Lor, Lor_true, xgrid, alphaD_calc, B_last, E_last = \
            train_step(params, tier_points_str, strength_train, alphaD_train)
        costs_hist.append(float(total.numpy()))

        if it % PRINT_EVERY == 0 or it == 1 or it == NUM_ITERS_TIER:
            plt.stem(E_last, B_last)
            plt.plot(xgrid, Lor)
            plt.plot(xgrid, Lor_true)
            print(f"[Tier {tier_idx}] iter {it:6d} | total={total.numpy():.6e} "
                  f"| S={Ls.numpy():.3e} | m-1={Lm1.numpy():.3e} | m+1={Lmplus1.numpy():.3e}")
            plt.show()

    # Evaluate on fixed test set
    test_total, test_Ls, test_Lm1, test_Lmplus1 = \
        evaluate_on(params, test_points_str, strength_test, alphaD_test)
        
        
    # Evaluate on train set
    train_total, train_Ls, train_Lm1, train_Lmplus1 = \
        evaluate_on(params, tier_points_str, strength_train, alphaD_train)

    print(f"[Tier {tier_idx}] Test: total={test_total:.6e} "
          f"| S={test_Ls:.3e} | m-1={test_Lm1:.3e} | m+1={test_Lmplus1:.3e}")
    
    # --- NEW: alphaD relative errors on test set ---
    save_dir = "alphaD_eval_Em1_"+str(PARAM_SEED)  # change if you want a different folder
    ...
    tier_results.append({
        ...
        "test_strength": test_Ls,
        "test_mminus1": test_Lm1,
        "test_mplus1": test_Lmplus1,
