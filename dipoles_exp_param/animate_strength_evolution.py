...
for idx in range(len(combined)):
    ...
    opt_D, opt_S1, opt_S2,opt_S3,opt_S4, opt_v0,opt_v1, opt_v2, fold, x1, x2, x3 = helper.modified_DS_affine_v(params, n)
    ...
    eta_new = tf.sqrt(fold**2 + (x2*alpha_tensor + x3*beta_tensor)**2)
...
fig, (ax_strength, ax_grid) = plt.subplots(2, 1, figsize=(8, 14), dpi=150, 
...
ax_grid.scatter(train_alpha, train_beta, marker='o', facecolors='none', edgecolors='black', s=60)
...
def animate(i):
    ...
    eta_new = tf.sqrt(fold**2 + (x2*alpha_tensor + x3*beta_tensor)**2)
    ax_grid.set_title(str(eta_new.numpy()))
    
    lor = helper.give_me_Lorentzian(x, eigvals_trunc, B, eta_new)
...
# Save MP4
ani.save(f"animate_strength_evolution_noeigs_{mode}_fixed_{fixed_value}_PMM{n}.mp4", writer='ffmpeg', fps=1.3)