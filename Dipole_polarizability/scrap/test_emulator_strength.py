...
'''
Lauren: Original script had test set??? Changed to train set for now to see if this was a typo.
'''
...
params = np.loadtxt(f'runs_em1/best_params_global.txt')
...
'''
idxs = [4,1,2]
x1, orig1, opt1 = helper_gpt.data_Lorentzian_for_idx(idxs[0], train_set,n,params)
x2, orig2, opt2 = helper_gpt.data_Lorentzian_for_idx(idxs[1], train_set,n,params)
x3, orig3, opt3 = helper_gpt.data_Lorentzian_for_idx(idxs[2], train_set,n,params)
plt.plot(x1, orig1, color = 'red', label ='$b_{TV} ='+train_set[idxs[0]][1][:4]+'$ fm${}^{-2}$, $d_{TV}='+train_set[idxs[0]][0][:4]+'$')
plt.plot(x2, orig2, color = 'blue', label ='$b_{TV} ='+train_set[idxs[1]][1][:4]+'$ fm${}^{-2}$, $d_{TV}='+train_set[idxs[1]][0][:4]+'$')
plt.plot(x3, orig3, color = 'green', label ='$b_{TV} ='+train_set[idxs[2]][1][:4]+'$ fm${}^{-2}$, $d_{TV}='+train_set[idxs[2]][0][:4]+'$')
