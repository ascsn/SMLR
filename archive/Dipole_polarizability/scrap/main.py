#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 15 16:19:05 2025

@author: anteravlic + formatting by ChatGPT
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Dipole Polarizability Emulator (EM-Dipole)

This script trains a low-dimensional emulator for dipole strength and polarizability.
It is designed for end users:
  - Runs headless (no GUI windows).
  - Figures are NEVER shown; use `--plots save` to write PNG/PDFs.
  - All per-run artifacts are saved under `<save-dir>/seed_<SEED>/`.
  - Enforces a minimum of 20,000 iterations per restart.

Example:
  # Fast default (headless, no figures)
  python dipole_main.py

  # Customized training with saved figures
  python dipole_main.py --n 10 --retain 0.5 --fold 2.0 \
                        --n-restarts 5 --seed0 100 \
                        --num-iter 40000 --print-every 500 \
                        --plots save --save-dir runs_em1
"""

# ------------------------- Imports  -------------------------
import os, re, argparse, random as rn
import numpy as np
import tensorflow as tf

import matplotlib
matplotlib.use("Agg")             # <- never opens windows
import matplotlib.pyplot as plt

import archive.Dipole_polarizability.scrap.helper as helper
''

# ------------------------------ CLI: user options ------------------------------
def parse_args():
    p = argparse.ArgumentParser(
        description="Train the dipole polarizability emulator (headless)."
    )
    # Model basics
    p.add_argument("--n",          type=int,   default=10,   help="Model dimension (default: 10)")
    p.add_argument("--retain",     type=float, default=0.5,  help="Fraction of eigenmodes kept (centered)")
    p.add_argument("--fold",       type=float, default=2.0,  help="Base Lorentzian width (eta)")

    # Restarts & training loop
    p.add_argument("--n-restarts", type=int,   default=10,   help="Number of restarts (different seeds)")
    p.add_argument("--seed0",      type=int,   default=42,   help="Base seed (seeds = seed0..seed0+n_restarts-1)")
    p.add_argument("--num-iter",   type=int,   default=30000,help="Iterations per restart (must be ≥ 20000)")
    p.add_argument("--print-every",type=int,   default=1000,  help="Logging cadence (iterations)")

    # Output / plotting
    p.add_argument("--save-dir",   type=str,   default="runs_em1", help="Directory for run artifacts")
    p.add_argument("--plots",      choices=["none","save"], default="save",
                   help="Skip plots (none) or save figures to disk (save).")

    return p.parse_args()


args = parse_args()

# Bind CLI to variables used below
n              = args.n
retain         = args.retain
fold           = args.fold
N_RESTARTS     = args.n_restarts
SEED0          = args.seed0
NUM_ITERATIONS = args.num_iter
PRINT_EVERY    = args.print_every
SAVE_DIR       = args.save_dir
PLOTS_MODE     = args.plots  # "none" | "save"

os.makedirs(SAVE_DIR, exist_ok=True)

# Training policy: hard minimum number of iterations per restart
MIN_ITERATIONS = 20000
if NUM_ITERATIONS < MIN_ITERATIONS:
    raise ValueError(f"--num-iter ({NUM_ITERATIONS}) must be >= {MIN_ITERATIONS}.")



# ------------------------------- Build the dataset -----------------------------
# User note: this reads strength/polarizability data from a fixed directory layout.
# Adjust the paths below if data folders differ.
rn.seed(142)
np.random.seed(30)

strength_dir = '../dipoles_data_all/total_strength/'
alphaD_dir   = '../dipoles_data_all/total_alphaD/'

# Filenames look like: strength_<beta>_<alpha>.out  (strings with decimal points)
pattern = re.compile(r'strength_([0-9.]+)_([0-9.]+)\.out')

formatted_alpha_values, formatted_beta_values, all_points = [], [], []
for fname in os.listdir(strength_dir):
    m = pattern.match(fname)
    if not m:
        continue
    beta_val  = m.group(1)
    alpha_val = m.group(2)
    all_points.append((alpha_val, beta_val))
    # Keep a rectangular region of the grid (tweak as needed for dataset)
    if (1.5 <= float(beta_val) <= 4.0) and (0.4 <= float(alpha_val) <= 1.8):
        formatted_alpha_values.append(alpha_val)
        formatted_beta_values.append(beta_val)

alpha = formatted_alpha_values
beta  = formatted_beta_values
combined = [(alpha[i], beta[i]) for i in range(len(alpha))]

if not combined:
    raise RuntimeError(f"No training points found in {strength_dir}. Check file names and filters.")

# Choose a central point: closest to the center of the bounding box
Amin = min(float(a) for a, b in combined); Amax = max(float(a) for a, b in combined)
Bmin = min(float(b) for a, b in combined); Bmax = max(float(b) for a, b in combined)
cx, cy = (Amin + Amax) / 2.0, (Bmin + Bmax) / 2.0
central_point = min(
    combined,
    key=lambda t: (float(t[0]) - cx)**2 + (float(t[1]) - cy)**2
)

# Single split (train only), as in the original code
train_set = combined
cv_set, test_set = [], []
print('Central point:', central_point)
print('Sizes -> test:', len(test_set), 'cv:', len(cv_set), 'train:', len(train_set))



# ---------------------------- Model & data preparation -------------------------
D, S1, S2 = helper.nec_mat(n)
print("Matrix shapes:", D.shape, S1.shape, S2.shape)

# Load strength & alphaD tables via project helper
strength, alphaD = helper.data_table(train_set)
alphaD = np.vstack(alphaD)            # shape: (N, 3) with last col alphaD
alphaD_list = [float(a[2]) for a in alphaD]

# Parameter count (same structure as original)
nec_num_param = 1 + 3*n + n + 4 * int(n * (n + 1) / 2) + 4
print('Number of parameters:', nec_num_param)

# --- Central-spectrum fit (used to seed the optimizer) ---
# We fit once to the central point and reuse the result for every seed.
central_file = os.path.join(strength_dir, f"strength_{central_point[1]}_{central_point[0]}.out")
data = np.loadtxt(central_file)
omega = data[:, 0].astype(np.float32)
y     = data[:, 1].astype(np.float32)

omega_tf = tf.convert_to_tensor(omega, dtype=tf.float32)
y_tf     = tf.convert_to_tensor(y,     dtype=tf.float32)
eta_tf   = tf.convert_to_tensor(fold,  dtype=tf.float32)

keep = round(retain * n)  # number of modes to keep
E_hat, B_hat, y_hat = helper.fit_strength_with_tf_lorentzian(
    omega_tf, y_tf, keep, eta_tf, min_spacing=0.01
)

def save_central_fit_plot(out_dir: str):
    """Optionally save a diagnostic plot of the central spectrum fit."""
    if PLOTS_MODE != "save":
        return
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.plot(data[:, 0], y_hat, label='fit')
    ax.stem(E_hat, B_hat, label='poles')
    ax.plot(data[:, 0], data[:, 1], label='true')
    ax.set_title('Central spectrum fit')
    ax.legend()
    fig.savefig(os.path.join(out_dir, "central_spectrum_fit.png"), bbox_inches='tight', dpi=150)
    plt.close(fig)

# Initial parameter guess (user-facing defaults)
def make_initial_guess(seed: int):
    rng = np.random.default_rng(seed)
    init = rng.uniform(0.0, 1.0, size=nec_num_param).astype(np.float32)
    init[0]   = fold
    # Final four entries encode linear width model (x1, x1, x2, x3) per original code
    init[-4:] = np.array([1, 1, 1, 1], dtype=np.float32)
    return init

def encode_init_with_fit(random_init):
    # Project-specific packing of poles/weights around the central fit
    return helper.encode_initial_guess(random_init, E_hat, B_hat, n, retain).astype(np.float32)



# ------------------------ Early stopping configuration -------------------------
# Two criteria: (A) "best" patience, (B) moving-average plateau.
# We also require a hard minimum iteration count (MIN_ITERATIONS).
PATIENCE_BEST        = 120      # logs without new "best"
MIN_DELTA_REL_BEST   = 2e-4     # relative improvement to reset best

PATIENCE_PLATEAU     = 120      # logs without MA improvement
MA_WINDOW            = 10       # length of moving average (in logs)
MIN_DELTA_REL        = 3e-4     # minimum relative MA improvement

WARMUP_LOGS          = 12       # skip early-stop counting during first logs

REQUIRE_BOTH_TO_STOP = True     # require BOTH criteria to trigger stop

def moving_average(arr, k):
    if len(arr) < k:
        return None
    return np.convolve(arr, np.ones(k)/k, mode='valid')



# ------------------------------- Training (restarts) ---------------------------
def set_all_seeds(seed: int):
    rn.seed(seed); np.random.seed(seed); tf.random.set_seed(seed)

global_best_cost   = np.inf
global_best_params = None
global_best_meta   = {"seed": None, "iter": None}

for r in range(N_RESTARTS):
    seed = SEED0 + r
    run_dir = os.path.join(SAVE_DIR, f"seed_{seed}")
    os.makedirs(run_dir, exist_ok=True)

    print(f"\n========== RESTART {r+1}/{N_RESTARTS} (seed={seed}) ==========")
    print(f"Policy: MIN_ITERATIONS = {MIN_ITERATIONS}, user max = {NUM_ITERATIONS}")
    set_all_seeds(seed)

    # Save per-seed diagnostic of the central fit (optional)
    save_central_fit_plot(run_dir)

    # Parameters & optimizer
    init_vec = make_initial_guess(seed)
    init_vec = encode_init_with_fit(init_vec)
    params   = tf.Variable(init_vec, dtype=tf.float32)
    optimizer = tf.keras.optimizers.Adam(learning_rate=0.01)

    @tf.function
    def optimization_step():
        with tf.GradientTape() as tape:
            (cost, strength_cost, alphaD_cost, m1_cost,
             Lor, Lor_true, x, alphaD_train, B, eigenvalues) = helper.cost_function_batched_mixed(
                params, n, train_set, strength, alphaD_list,
                central_point, retain,
                1, 2, 0, 875.0, 1e-8   # project-specific weights/args
            )
        grads = tape.gradient(cost, [params])
        optimizer.apply_gradients(zip(grads, [params]))
        return (cost, strength_cost, alphaD_cost, m1_cost,
                Lor, Lor_true, x, alphaD_train, B, eigenvalues)

    cost_history = []
    best_cost_this   = np.inf
    best_params_this = None
    best_iter_this   = -1

    # Early-stop trackers
    logs_done     = 0
    no_improve_cnt = 0
    plateau_cnt    = 0
    last_ma        = None

    for i in range(NUM_ITERATIONS):
        (cost, strength_cost, alphaD_cost, m1_cost,
         Lor, Lor_true, x, alphaD_train, B, eigs) = optimization_step()
        c = float(cost.numpy())
        cost_history.append(c)

        # Track "best"
        rel_impr_best = (best_cost_this - c) / max(abs(best_cost_this), 1e-12) if best_cost_this < np.inf else np.inf
        improved = (rel_impr_best >= MIN_DELTA_REL_BEST) or (best_cost_this == np.inf)
        if improved:
            best_cost_this   = c
            best_params_this = params.numpy().copy()
            best_iter_this   = i
            no_improve_cnt   = 0
        else:
            no_improve_cnt  += 1

        # Logging + moving-average checks at cadence
        if (i % PRINT_EVERY) == 0:
            logs_done += 1
            try:
                lr = float(optimizer._decayed_lr(tf.float32).numpy())
            except Exception:
                lr = 0.01

            rel = np.abs(np.array(alphaD_train) - np.array(alphaD[:, 2])) / np.array(alphaD[:, 2])
            print(f"[seed {seed}] iter {i:6d} | cost={c:.6e} | lr={lr:.3e} | "
                  f"no_improve={no_improve_cnt} | mean alphaD rel={np.mean(rel):.3e} | "
                  f"str%={float(strength_cost/c):.3f} alphaD%={float(alphaD_cost/c):.3f} m1%={float(m1_cost/c):.3f}")

            # Warm-up & hard floor: do not consume patience early or before MIN_ITERATIONS
            if logs_done <= WARMUP_LOGS or (i + 1) < MIN_ITERATIONS:
                no_improve_cnt = 0; plateau_cnt = 0; last_ma = None
            else:
                ds = cost_history[::PRINT_EVERY] if PRINT_EVERY > 0 else cost_history[:]
                ma = moving_average(ds, MA_WINDOW)
                if ma is not None:
                    current_ma = ma[-1]
                    if last_ma is None:
                        last_ma = current_ma
                        plateau_cnt = 0
                    else:
                        rel_impr_ma = (last_ma - current_ma) / max(abs(last_ma), 1e-12)
                        if rel_impr_ma >= MIN_DELTA_REL:
                            plateau_cnt = 0; last_ma = current_ma
                        else:
                            plateau_cnt += 1

        # Early-stop decision (after MIN_ITERATIONS only)
        stop_best    = (no_improve_cnt >= PATIENCE_BEST)
        stop_plateau = (plateau_cnt    >= PATIENCE_PLATEAU)
        can_stop     = (i + 1) >= MIN_ITERATIONS
        do_stop = can_stop and ((stop_best and stop_plateau) if REQUIRE_BOTH_TO_STOP
                                else (stop_best or stop_plateau))
        if do_stop:
            reason = ("both rules" if REQUIRE_BOTH_TO_STOP
                      else "no best improvement" if stop_best else "moving-average plateau")
            print(f"[seed {seed}] Early stopping at iter {i} due to {reason}. "
                  f"best={best_cost_this:.6e} @ {best_iter_this}")
            break



    # --------------------- Save artifacts for this restart ---------------------
    np.savetxt(os.path.join(run_dir, f"params_n{n}_retain{retain}_seed{seed}.txt"), best_params_this)
    np.savetxt(os.path.join(run_dir, f"cost_history_seed{seed}.txt"), np.array(cost_history))
    with open(os.path.join(run_dir, "meta.txt"), "w") as f:
        f.write(f"best_cost={best_cost_this}\n")
        f.write(f"best_iter={best_iter_this}\n")
        f.write(f"seed={seed}\n")
        f.write(f"n={n}\nretain={retain}\nfold={fold}\n")
        f.write(f"PRINT_EVERY={PRINT_EVERY}\n")
        f.write(f"PATIENCE_BEST={PATIENCE_BEST}, MIN_DELTA_REL_BEST={MIN_DELTA_REL_BEST}\n")
        f.write(f"PATIENCE_PLATEAU={PATIENCE_PLATEAU}, MA_WINDOW={MA_WINDOW}, MIN_DELTA_REL={MIN_DELTA_REL}\n")
        f.write(f"MIN_ITERATIONS={MIN_ITERATIONS}\n")

    # Convergence plot (saved only if requested)
    if PLOTS_MODE == "save":
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.plot(range(len(cost_history)), cost_history, label=f'seed {seed}')
        # Optional moving-average overlay
        ds = cost_history[::PRINT_EVERY] if PRINT_EVERY > 0 else cost_history[:]
        ma = moving_average(ds, MA_WINDOW)
        if ma is not None:
            xs = np.arange(len(ma)) * PRINT_EVERY + PRINT_EVERY*(MA_WINDOW-1)//2
            ax.plot(xs, ma, linestyle='--', label='moving avg')
        ax.set_yscale('log'); ax.set_xlabel('Iteration'); ax.set_ylabel('Cost')
        ax.set_title(f'Convergence (seed={seed}) | best={best_cost_this:.3e} @ iter {best_iter_this}')
        ax.legend()
        fig.savefig(os.path.join(run_dir, f'cost_curve_seed{seed}.png'), bbox_inches='tight', dpi=150)
        # fig.savefig(os.path.join(run_dir, f'cost_curve_seed{seed}.pdf'), bbox_inches='tight')
        plt.close(fig)

    # Track global best across restarts
    if best_cost_this < global_best_cost:
        global_best_cost   = best_cost_this
        global_best_params = best_params_this.copy()
        global_best_meta   = {"seed": seed, "iter": best_iter_this}



# -------------------------- Save global best & summary -------------------------
np.savetxt(f'params_best_n{n}_retain{retain}.txt', global_best_params)
print(f"\n*** GLOBAL BEST *** cost={global_best_cost:.6e} "
      f"(seed={global_best_meta['seed']}, iter={global_best_meta['iter']})")
print(f"Saved per-seed runs in {SAVE_DIR}/seed_<SEED>/ and best as params_best_{n}_retain{retain}.txt")

# Save the training grid for reference
with open("train_set.txt", "w") as f:
    for tup in train_set:
        f.write(",".join(map(str, tup)) + "\n")
