...
v0 = np.random.uniform(0, 1, n) #np.random.rand(n)

'''
    data table is now constructed for alpha & beta parameters
'''

Lors, HLs = helper.data_table(train_set, coeffs, g_A)
...
def optimization_step():
    with tf.GradientTape() as tape:
        cost, Lor, Lor_true,x, HLs_calc, eigenvalues, B = helper.cost_function(params, n, train_set\
                                              , Lors,HLs, coeffs, g_A, weight, width)
    gradients = tape.gradient(cost, [params])
    optimizer.apply_gradients(zip(gradients, [params]))
    return cost, Lor, Lor_true, x, eigenvalues, B
...
def cv_step():
    ...
    '''
    cost, Lor, Lor_true,x, HLs_calc, eigenvalues,B = helper.cost_function(params, n, cv_set\
                                      , Lors_cv, HLs_cv, coeffs, g_A, weight, width)
  
    return cost, HLs_calc

# Run the optimization
num_iterations = 50000 # Total number of optimization iterations
num_check = 300 #Print output after this many steps
cost_loop = []
cost_loop_cv = []
for i in range(num_iterations):
    cost, Lor, Lor_true, x, eigenvalues, B = optimization_step()
    ...
    if (np.mean(rel) < 0.0010):
        print('Final:', np.mean(rel))
        break
