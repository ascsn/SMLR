...
for i in range(num_iterations):
    ...
    if i % num_check == 0:  # Print cost every 100 iterations
        ...
        print('Train cost: ', cost.numpy())
        print('r: ', np.mean(rel))
        plt.figure(i)
        rel =  np.abs(np.array(HLs_train)-np.array(HLs))/np.array(HLs)
        plt.plot([j for j in range(len(train_set))],HLs_train, marker = '.', label = 'Emu', ls = '--', color = 'red') 
        plt.plot([j for j in range(len(train_set))],HLs, marker = '.', label = 'QRPA', ls = '--',color = 'black') 
        plt.yscale('log')
...
plt.yscale('log')
print('Final: ', train_cost_min.numpy())
print('At iteration: ', min_train_iter)
...
np.savetxt('params_'+str(n)+'_only_HL.txt', params_min.numpy())
