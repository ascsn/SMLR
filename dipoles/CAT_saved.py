...
for n in range(len(num_par)):
    ...
    plt.figure(3)
    rel =  data[:,1]
    plt.scatter(data[:,0]*1e3, rel, label = 'n(Alg 1) = '+str(num_par[n]), alpha = 0.8, color = colors[n])
...
for n in range(len(num_par)):
    ...
    plt.figure(3)
    rel =  data[:,1]
    plt.scatter(data[:,0]*1e3, rel, label = 'n(Alg 2) = '+str(num_par[n]), alpha = 0.8, marker = 'x',
                color = colors[n])
